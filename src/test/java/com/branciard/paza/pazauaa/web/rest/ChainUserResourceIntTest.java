...
public class ChainUserResourceIntTest {
    ...
    public void createChainUser() throws Exception {
        ...
        List<ChainUser> chainUserList = chainUserRepository.findAll();
        assertThat(chainUserList).hasSize(databaseSizeBeforeCreate + 1);
        ChainUser testChainUser = chainUserList.get(chainUserList.size() - 1);
    ...
    @Test
    @Transactional
    public void createChainUserWithExistingId() throws Exception {
        int databaseSizeBeforeCreate = chainUserRepository.findAll().size();

        // Create the ChainUser with an existing ID
        ChainUser existingChainUser = new ChainUser();
        existingChainUser.setId(1L);

        // An entity with an existing ID cannot be created, so this API call must fail
        restChainUserMockMvc.perform(post("/api/chain-users")
            .contentType(TestUtil.APPLICATION_JSON_UTF8)
            .content(TestUtil.convertObjectToJsonBytes(existingChainUser)))
            .andExpect(status().isBadRequest());

        // Validate the Alice in the database
        List<ChainUser> chainUserList = chainUserRepository.findAll();
        assertThat(chainUserList).hasSize(databaseSizeBeforeCreate);
    }
    ...
    public void checkEnrollmentIdIsRequired() throws Exception {
        ...
        List<ChainUser> chainUserList = chainUserRepository.findAll();
        assertThat(chainUserList).hasSize(databaseSizeBeforeTest);
    }
    ...
    public void updateChainUser() throws Exception {
        ...
        // Validate the ChainUser in the database
        List<ChainUser> chainUserList = chainUserRepository.findAll();
        assertThat(chainUserList).hasSize(databaseSizeBeforeUpdate);
        ChainUser testChainUser = chainUserList.get(chainUserList.size() - 1);
    ...
    @Test
    @Transactional
    public void updateNonExistingChainUser() throws Exception {
        int databaseSizeBeforeUpdate = chainUserRepository.findAll().size();
        ...
        // Validate the ChainUser in the database
        List<ChainUser> chainUserList = chainUserRepository.findAll();
        assertThat(chainUserList).hasSize(databaseSizeBeforeUpdate + 1);
    }
    ...
    public void deleteChainUser() throws Exception {
        ...
        List<ChainUser> chainUserList = chainUserRepository.findAll();
        assertThat(chainUserList).hasSize(databaseSizeBeforeDelete - 1);
    }
}
